...
M.open_journal = function(notes_root_path, journal_dir_name)
	assert(notes_root_path, "notes_root_path must be configured")
	assert(journal_dir_name, "journal_dir_name must be configured")
			...
			attach_mappings = function(prompt_bufnr, _)
				actions.select_default:replace(function()
					...
					local journal_file_path = vim.fn.expand(
						vim.fn.resolve(notes_root_path .. "/" .. journal_dir_name .. "/" .. journal_file_name)
					)
...
M.open_daily_journal = function(notes_root_path, journal_dir_name)
	assert(notes_root_path, "notes_root_path must be configured")
	assert(journal_dir_name, "journal_dir_name must be configured")
	...
	local daily_note_file_path =
		vim.fn.expand(vim.fn.resolve(notes_root_path .. "/" .. journal_dir_name .. "/" .. daily_note_file_name))
