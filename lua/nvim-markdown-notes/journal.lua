      ...
      .new({}, {
        ...
        attach_mappings = function(prompt_bufnr, _)
          actions.select_default:replace(function()
            ...
            if type(journal_file_path) == "string" and vim.fn.filereadable(journal_file_path) == 1 then
              vim.cmd("edit " .. journal_file_path)
            else
              local file_path = notes.create_note(selection.value, M.opts.journal_dir_path, selection.value)
              if file_path then
